...
public class StoreDataJob implements BatchJob {
		...
		try {
			...
			try {			
				...
				try {
					...
					crons = jdbcTemplate.query(props.getProperty("cron.job.select.data.processing.job"), new Object[]{jobId}, new DataProcessingCronsMapper());
				} catch(Exception e) {
	...
	private void recordError(JobState jobState, String dataProcessingJobId, JdbcTemplate jdbcTemplate) {
	    ...
	    Boolean success = jdbcTemplate.execute("UPDATE data_processing_crons set already_exist = ?, success = ?, failed = ?, last_processed_key = ? where id = ?", psFunction);
	}
}
