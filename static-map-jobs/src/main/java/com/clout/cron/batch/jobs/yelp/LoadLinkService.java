...
public class LoadLinkService  { 
			...
			for(Naics naic : naicsCategory) {
				...
				if(pages > 1) {
					for(int i = 2; i < pages; i ++) {
						try {
							YelpInfo info = new YelpBuilder().setNaics(naic).setCategory(naic.getCodeDetails()).setLocation(city + ", " + state).setPage(i+"").execute(); 
							jdbcTemplate.execute("INSERT INTO yelp_links (naics_code, code_detail,link, city, state, category, page) VALUES ('" + info.getNaics().getCode() + "', '" + info.getNaics().getCodeDetails() + "','" + info.getServiceEndpoint() + "','" + city + "','" + state + "','" + naic.getCodeDetails() + "', '" + i + "');");
	...
	private int buildInfos(Naics naic, String page, String city, String state) {
		try {
			...
			jdbcTemplate.execute("INSERT INTO yelp_links (naics_code, code_detail,link, city, state, category, page) VALUES ('" + info.getNaics().getCode() + "', '" + info.getNaics().getCodeDetails() + "','" + info.getServiceEndpoint() + "','" + city + "','" + state + "','" + naic.getCodeDetails() + "', '" + page + "');");
