	...
	public void execute() throws CronJobException {
		...
		try {
			...
			try {
				 ...
				 */
				storeJdbcTemplate.query(categoryQuery, (rs, rowNum) -> {
				...
				logger.info("categories size " + categoryLevel2Mapping.size());
				for(Map.Entry<Integer, Integer> entry : categoryLevel2Mapping.entrySet()) { 
					logger.info("categoryid " + entry.getKey() + " category names size " + entry.getValue());
				}
				/*
				 * end category map
				 */
				...
				try {
					Integer maxId = jobJdbcTemplate.queryForObject("select max(id) from data_processing_crons where job_name = ?", new Object[]{jobName}, Integer.class);
					crons = jobJdbcTemplate.query("select * from data_processing_crons where id = ?", new Object[]{maxId}, new DataProcessingCronsMapper());
